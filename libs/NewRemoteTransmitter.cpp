...
NewRemoteTransmitter::NewRemoteTransmitter(unsigned long address, int8_t pin, unsigned int periodusec, int8_t repeats) {
	_address = address;
	_pin = pin;
...
void NewRemoteTransmitter::sendUnit(int8_t unit, bool switchOn) {
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
...
void NewRemoteTransmitter::sendDim(int8_t unit, int8_t dimLevel) {
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
...
void NewRemoteTransmitter::sendGroupDim(int8_t dimLevel) {
	for (int8_t i = _repeats; i >= 0; i--) {
		_sendStartPulse();
...
void NewRemoteTransmitter::_sendUnit(int8_t unit) {
