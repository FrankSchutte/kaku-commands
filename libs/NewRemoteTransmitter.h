...
class NewRemoteTransmitter {
	public:
		...
		*/
		NewRemoteTransmitter(unsigned long address, int8_t pin, unsigned int periodusec = 260, int8_t repeats = 4);
		 ...
		 * @param switchOn  True to send "on" signal, false to send "off" signal.
		 */
		void sendUnit(int8_t unit, bool switchOn);
		 ...
		 */
		void sendDim(int8_t unit, int8_t dimLevel);
		 ...
		 * @param dimLevel  [0..15] Dim level. 0 for off, 15 for brightest level.
		 */
		void sendGroupDim(int8_t dimLevel);

	protected:
		unsigned long _address;		// Address of this transmitter.
		int8_t _pin;				// Transmitter output pin
		unsigned int _periodusec;	// Oscillator period in microseconds
		int8_t _repeats;			// Number over repetitions of one telegram
